...
class BusinessVerify extends StatefulWidget {
  ...
  const BusinessVerify({Key? key, required this.itemResponse})
      : super(key: key);

  @override
  _BusinessVerifyState createState() => _BusinessVerifyState();
}

class _BusinessVerifyState extends State<BusinessVerify> {
  Map newItem = {};

  var screen = "Verify Business";
  ...
  late File selectedOtherFile;
  late File selectedAadharFrontFile;
  late File selectedAadharBackFile;
  ...
  cropImageFunction(File file, filename, docType) async {
    ...
    setState(() {
      ...
      _upload(croppedFile, filename, docType);
    });
  }

  void _upload(File file, filename, docType) async {
    setState(() {
      docUploading = true;
    });
    ...
    printMessage(screen, "Fields : ${data.fields.toString()}");
